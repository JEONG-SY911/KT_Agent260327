...
const PDF_TYPES = [
  { id: 1, label: "① 시장 진입 브리핑",    emoji: "📄", desc: "내부 경영진 보고용" },
  { id: 2, label: "② 경쟁 전략 요약",      emoji: "📋", desc: "제안/입찰용" },
  { id: 3, label: "③ 투자자용 시장 요약",  emoji: "💹", desc: "VC·투자자용" },
  { id: 4, label: "④ 파트너십 제안서",     emoji: "🤝", desc: "파트너사 첨부용" },
  { id: 5, label: "⑤ 신사업 검토 보고",   emoji: "🔍", desc: "전략기획·임원 보고" },
] as const;
...
const S = {
  ...
  dot:     "w-1 h-1 rounded-full bg-gray-500 mt-1.5 flex-shrink-0",
...
export function PdfExportButtons({ result }: Props) {
  ...
  return (
    <div>
      {/* ── 버튼 그룹 ─────────────────────────────────────── */}
      <div className="mb-4">
        <p className="text-xs text-slate-500 mb-2 font-medium">PDF 내보내기</p>
        <div className="flex flex-wrap gap-2">
          {PDF_TYPES.map(({ id, label, emoji, desc }) => (
            <button
              key={id}
